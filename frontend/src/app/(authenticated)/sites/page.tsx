...
import { useSites } from '@/modules/sites/hooks/use-sites';
import { Card, Button, Badge, Spinner } from '@/shared/ui';
import { useAuth } from '@/shared/auth/auth-context';

export default function SitesPage() {
  const [search, setSearch] = useState('');
  const { sites, isLoading, error, updateFilter } = useSites();
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <h1 className="text-2xl font-semibold text-slate-900 dark:text-white">Site Registry</h1>
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                {sites.map((site) => (
                  <tr key={site._id || site.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                    ...
                    <td className="px-4 py-3 truncate max-w-[200px]">{site.address}</td>
